...
假定你有一个资源N个不同的定义，Android是如何选择一个的？

首先，Android弃用掉那些明确非法的。所以，例如，如果设备的语言是-ru,Android会忽略
指定为其它语言(例如，-zh)的资源集。这个的例外是密度限定和屏幕尺寸限定——我们稍后
会了解这些例外。

然后，Android选择那个有着要求的资源和最重要唯一限定符的资源集。这里，通过”最重要“
我们意味着出现在目录名最左侧的那个，基于以上所讨论的目录命名规则。并且，通过“唯一”，
我们意味着没有其它资源有这个限定符。

如果没有对应的特定资源集，Android会选择默认的集合——那个目录名中没有后缀的(例如，res/layout/)。

记住了这些规则，让我们看看一些场景，除了基本情况还涵盖上述的例外。
...
当我们调用setContentView(R.layout.main),如果设备是处于水平模式的话Android会选择
位于res/layout-land/的main.xml。在此例中这个特定的资源集是合法的，并且它有着最重要的
唯一限定符(-land)。如果设备是处于竖直模式，res/layout-land/资源集就不能胜任了,并且所以
它就被丢弃了。这就给我们剩下了res/layout/,所以Android使用了这个main.xml版本。

####场景#2:毫不相干的资源集种类
尽管很奇怪，对于你来说存在一个项目带有main.xml位于：
...
”语言和地域“比"屏幕方向"的优先级要来的高。尽管如此，如果设备没有设置成英语，Android会丢弃掉这个
资源集，就这一点来说决策过程是和以上的场景#1一样的。

####场景#3:多样的限定符

让我们设想一个项目，其中main.xml存在于：
...
决策过程是和以上的场景#2类似的：Android针对英语会选择res/layout-en/,
针对水平的API等级11以上的设备会选择res/layout-land-v11,又或者
所有其它的会选择res/layout/。
...
如果你有着和屏幕尺寸绑定的资源集，Android会选择最接近实际屏幕尺寸依然比
实际尺寸小的。比实际尺寸大的屏幕尺寸的资源集是被忽略的。

对于所有设备来说，这适合-swNNNdp,-wNNNdp,以及-hNNNdp。在-large或者
...
如果设备是水平的，Android会选择res/layout-land。如果所有其它的都失败了，
Android会选择res/layout/。
...
否则的话，如果设备是水平的，Andorid会选择res/layout-land。如果所有其它的都失败了，
Android会选择res/layout/。
